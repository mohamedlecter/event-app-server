...
exports.verifyPayment = async (req, res) => {
    try {
      ...
      if (event[ticketField].sold + payment.tickets.length > event[ticketField].quantity) {
    ...
    await Ticket.updateMany(
      ...
      await Event.findByIdAndUpdate(payment.event, {
        ...
        $set: { 
          soldOut: event.standardTicket.sold >= event.standardTicket.quantity && 
...
exports.transferTicket = async (req, res) => {
  try {
    ...
    if (ticket.status !== "success") {
      return res.status(400).json({ message: "Only paid tickets can be transferred" });
    }
    ...
    res.status(500).json({ message: "Ticket transfer failed", error: error.message });
  }
};